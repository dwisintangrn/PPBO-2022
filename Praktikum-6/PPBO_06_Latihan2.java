...
public class PPBO_06_Latihan2 {
    public static void main(String[] args) {
        ...
        HashMap<String,Mahasiswa> MhsMap = new HashMap<String, Mahasiswa>();
        for(Mahasiswa mahasiswa : mahasiswaArr){
            MhsMap.put(mahasiswa.NIM, mahasiswa);
        }

        for(HashMap.Entry<String, Mahasiswa> set : MhsMap.entrySet()){
            System.out.printf("key: %s => data mahasiswa: %s, %s, %d\n", 
            set.getKey(), set.getValue().nama, set.getValue().NIM, set.getValue().angkatan);
        }
    }
}
